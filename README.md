...
Faro is a governed outreach intelligence workspace for partnership, sponsorship, fundraising,
program, and community teams. It turns fragmented relationship data into a clear next action,
explains the timing behind that recommendation, and keeps every AI-assisted draft under human
review.
...
The original scenario is hackathon sponsorship, but the same loop applies to nonprofit
fundraising, university partnerships, business development, customer success, and other
relationship-driven work. See
[Hackathon story and real-world applications](docs/HACKATHON_AND_USE_CASES.md).

## Interface

The screenshots below are captured from the running fictional workspace. They document implemented
UI; they are not generated feature mockups.

### Decision-first dashboard

![Faro dashboard with a priority relationship signal, follow-up queue, timing evidence, and campaign context](docs/screenshots/dashboard.png)

### Human-reviewed follow-up

![Faro follow-up center with explainable outreach timing and an editable demo draft](docs/screenshots/follow-ups.png)

<details>
<summary>Analytics and Google Sheets</summary>

![Faro analytics with response trend, funnel, timing heatmap, and campaign comparison](docs/screenshots/analytics.png)

![Faro Google Sheets mapping, validation preview, and sync history](docs/screenshots/google-sheets.png)
...
```mermaid
flowchart LR
  Context["Governed relationship context"] --> Faro["Faro decision layer"]
  Faro --> Queue["Prioritized follow-ups"]
  Faro --> Timing["Explainable outreach windows"]
  Queue --> Request["Scoped IBM Bob request"]
  Timing --> Request
  Request --> Review["Human review and approval"]
...
- **Notifications:** internal reminders remain distinct from external outreach. Preview is the
  default adapter; SMS requires an explicitly verified Twilio configuration and recipient.

See [Google Sheets](docs/GOOGLE_SHEETS.md),
[Gmail outreach and IBM Bob](docs/EMAIL_OUTREACH_AND_BOB.md), and
...
- [100 × 100 Faro project logo](docs/assets/faro-project-logo.png)
...
- [Hackathon narrative and use cases](docs/HACKATHON_AND_USE_CASES.md)

`PLAN.md` records the repository assessment, milestones, risks, and verification plan. `AGENTS.md`
contains the durable engineering conventions for contributors and coding agents.
